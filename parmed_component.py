...
class MolToParmedComponent(GenericComponent):
    ...
    def execute(
        ...
        return True, ParmedMol(mol=pmol)
...
class ParmedToMolComponent(GenericComponent):
    ...
    def execute(
        ...
        if hasattr(pmol.mol.atoms[0], "residue"):
            residues = [
                (atom.residue.name, atom.residue.idx) for atom in pmol.mol.atoms
            ]
...
class ParmedToFFComponent(GenericComponent):
    ...
    def execute(
        ...
        if inputs.args:
            if "software" in inputs.args:
                if inputs.args["software"] == "gromacs":
                    if len(ff.ff.itps) > 1:
                        raise ValueError(
                            f"More than 1 forcefield name detected: {ff.ff.itp}."
                        )
                    else:
                        print(
                            f"Forcefield {ff.ff.itps[0].split('/')[0].split('.ff')[0]} detected."
                        )
        ...
        dihedrals = [
            ...
            for dihedral in ff.ff.dihedrals
        ]
